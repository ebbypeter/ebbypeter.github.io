...
Create any file whose first line is `.LOG`, and dotLOG will automatically append a timestamp every time you open the file, positioning your cursor at the end ready for your next entry.
...
**The trigger condition** is simple: when a document is opened, dotLOG checks whether the filename starts with `.LOG` (case-insensitive). If it does not match, dotLOG does nothing and gets out of the way entirely.
...
2. Create a new file — name it anything. Add the first line as `.LOG`.
...
If you find a bug, open an issue. If you have an idea — timestamp format options, perhaps, or configurable trigger patterns — pull requests are welcome. Keep in mind that the guiding philosophy is deliberate minimalism: every feature addition should be measured against whether it preserves the essential zero-friction nature of the tool.

---

## Closing Thought

The `.LOG` trick in Notepad is a masterclass in quiet design. One line of text. No UI. No documentation needed. Completely discoverable by accident. Useful enough to survive unchanged for over thirty years.
